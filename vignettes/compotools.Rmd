...
sd(nreads)
```
Since the number of sequencing reads varied greatly across samples, these count data should not be used directly in a standard regression analysis. So we transform them into compositional data and then log them. As we are to use log-contrast model to analyse the data, we replace zero counts by the maximum rounding error 0·5 before transforming them.
...
refitted_model = cdmm(yt, xt[,selected], lam = 0)
refitted_model
```
Use refitted model to predict bmi.
```{r predict, include=TRUE, fig.width=6, fig.height=6, dpi=300}
yp <- refitted_model$int + log(as.matrix(xv[, selected])) %*% refitted_model$bet
ran = range(c(yv, yp))
...
### PrecisionMatrixEstimationforCompositionalData

```{r cv.care}
cv.care.r = cv.care(xle)
```
```{r care}
care.r = care(xle, lam = cv.care.r$lam)
```
